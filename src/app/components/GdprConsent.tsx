...
export function GdprConsent({ onConsent }: GdprConsentProps) {
  ...
  const handleConsent = () => {
    try { localStorage.setItem('reeveos_gdpr_v1', '1'); } catch {}
    onConsent();
  };
    ...
    <div style={{
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch' as any, overscrollBehavior:'contain' as any, padding:'0 24px' }}>
        ...
        <p style={{ fontSize:13, fontWeight:500, color:'#8E8E93', lineHeight:1.5, margin:'0 0 16px' }}>
          We process your data under UK GDPR / ICO law.
        </p>
        ...
        <div style={{ backgroundColor:'#F2F2F7', borderRadius:10, padding:'12px 14px', marginBottom:10 }}>
          ...
          {[['Account details','Name & email'],['Business data','Bookings, clients, services'],['Usage analytics','To improve the product']].map(([l,d],i) => (
            <div key={i} style={{ display:'flex', gap:8, marginBottom:i<2?6:0, alignItems:'flex-start' }}>
              ...
              <div><span style={{ fontSize:13, fontWeight:600, color:BRAND.black }}>{l}</span><span style={{ fontSize:12, color:'#8E8E93', marginLeft:4 }}>— {d}</span></div>
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor:'#F2F2F7', borderRadius:10, padding:'12px 14px', marginBottom:16 }}>
          ...
          {['Access, correct, or delete your data anytime','Withdraw consent and close your account','Request a full data export','Complain to the ICO (ico.org.uk)'].map((t,i) => (
      ...
      <div style={{ flexShrink:0, padding:'0 24px', paddingBottom:'max(32px, env(safe-area-inset-bottom, 32px))', backgroundColor:BRAND.white, borderTop:'1px solid #F0F0F0' }}>
        ...
        <button onClick={handleConsent} disabled={!accepted} style={{
